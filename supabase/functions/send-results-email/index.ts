...
<p style="font-size:12px;color:#9CA3AF;margin-top:32px;">Diagnostic généré par DÉCLIC — <a href="https://lecko.fr" style="color:#2563EB;">lecko.fr</a></p>
    ...
    const CONSULTANT_EMAIL = Deno.env.get("CONSULTANT_NOTIFICATION_EMAIL") ?? "contact@lecko.fr";
...
function buildConsultantBriefHtml(data: {
  ...
  const tasksHtml = (data.topTasks ?? []).map((t, i) =>
    `<tr><td style="padding:8px 12px;border-bottom:1px solid #F3F4F6;">
      <span style="display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;border-radius:5px;background:#EEF2FF;color:#2563EB;font-size:10px;font-weight:700;margin-right:8px;">${i+1}</span>
...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;padding-bottom:16px;border-bottom:2px solid #2563EB;">
  <div><span style="font-size:16px;font-weight:700;color:#2563EB;">DECLIC</span><span style="font-size:10px;color:#9CA3AF;margin-left:6px;">Nouveau lead</span></div>
...
<div style="background:#F8FAFC;border-radius:12px;padding:18px;margin-bottom:20px;">
  ...
  <p style="font-size:13px;color:#2563EB;margin:0 0 12px;"><a href="mailto:${data.email}" style="color:#2563EB;">${data.email}</a></p>
...
<div style="text-align:center;padding:20px 0;">
  <a href="mailto:${data.email}?subject=Suite de votre cartographie DECLIC — ${data.metier}" style="display:inline-block;padding:12px 28px;background:#2563EB;color:#fff;font-size:13px;font-weight:600;border-radius:9px;text-decoration:none;">Repondre a ${(data.nom ?? "").split(" ")[0] || "ce lead"}</a>
</div>
<p style="font-size:11px;color:#D1D5DB;text-align:center;margin-top:16px;">Lead genere via DECLIC by Lecko</p>
</body></html>`;
}
