...
const corsHeaders = {
  "Access-Control-Allow-Origin": "https://declic.lecko.fr",
  ...
  for (const data of emailsToSend) {
    try {
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: "DECLIC by Lecko <declic@lecko.fr>",
          to: data.email,
...
function buildHtml(d: any): string {
  ...
  const nextHtml = d.nextTasks.length > 0
    ? `<div style="margin:24px 0;"><p style="font-size:13px;font-weight:600;color:#374151;margin:0 0 12px;text-transform:uppercase;letter-spacing:0.05em;">Prochaines etapes</p>${d.nextTasks.map((t: string, i: number) => `<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid #F3F4F6;"><span style="display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:6px;background:#EEF2FF;color:#2563EB;font-size:11px;font-weight:700;">${i + 1}</span><span style="font-size:13px;color:#374151;">${t}</span></div>`).join("")}</div>`
    : "";
...
<div style="margin-bottom:32px;"><span style="font-size:18px;font-weight:700;color:#2563EB;">DECLIC</span><span style="font-size:10px;color:#9CA3AF;margin-left:8px;">by Lecko</span></div>
...
<td style="text-align:center;padding:8px 12px;"><div style="font-size:28px;font-weight:700;color:#2563EB;">${d.inProgressTasks}</div><div style="font-size:11px;color:#6B7280;margin-top:3px;">En cours</div></td>
...
${nextHtml}
<div style="text-align:center;margin:28px 0;"><a href="https://declic.lecko.fr/mes-automations" style="display:inline-block;padding:13px 28px;background:#2563EB;color:#fff;font-size:14px;font-weight:600;border-radius:10px;text-decoration:none;">Voir mes automations</a></div>
<p style="font-size:12px;color:#9CA3AF;text-align:center;margin:32px 0 0;line-height:1.6;">Vous recevez cet email car vous avez un diagnostic actif sur DECLIC.<br><a href="https://declic.lecko.fr/parametres" style="color:#9CA3AF;">Se desabonner</a></p>
</body></html>`;
}
